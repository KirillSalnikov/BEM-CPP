...
def try_flip(vertices, faces, edge, adjacent):
    f0, f1 = adjacent
    ...
    new0 = oriented([c, d, a], vertices, ref0)
    new1 = oriented([d, c, b], vertices, ref1)
...
def repair(mesh, target_angle, max_passes):
    ...
    for _ in range(max_passes):
        ...
        for edge in candidate_edges:
            ...
            if len(adjacent) != 2:
                continue
            result = try_flip(vertices, faces, edge, adjacent)
            if result is None:
                continue
...
def main():
    ...
    print({"flips": flips, **stats(repaired)})
