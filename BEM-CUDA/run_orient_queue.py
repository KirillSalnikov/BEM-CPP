...
RESUME_SIGNATURE_KEY = "_queue_resume_signature"
...
def optional_number(value, cast=float):
    return None if value is None else cast(value)


def build_resume_signature(args, orient_line_file):
    return {
        "version": 1,
        "executable_sha256": file_sha256(args.exe),
        "orientation_source_sha256": file_sha256(orient_line_file),
        "ka": float(args.ka),
        "ri": [float(args.ri[0]), float(args.ri[1])],
        "shape": args.shape,
        "obj_path": os.path.normpath(args.obj) if args.obj else None,
        "obj_sha256": file_sha256(args.obj),
        "subdiv": int(args.subdiv),
        "prism_aspect": float(args.prism_aspect),
        "edge_refine": args.edge_refine,
        "ref": int(args.ref),
        "quad": optional_number(args.quad, int),
        "ntheta": int(args.ntheta),
        "scat_plane": args.scat_plane,
        "alpha_avg": int(args.alpha_avg),
        "solver": args.solver,
        "accurate": bool(args.accurate),
        "fast_obj": bool(args.fast_obj),
        "system": args.system,
        "fmm_digits": optional_number(args.fmm_digits, int),
        "gmres_tol": optional_number(args.gmres_tol, float),
        "gmres_restart": optional_number(args.gmres_restart, int),
        "krylov": args.krylov,
        "max_leaf": optional_number(args.max_leaf, int),
        "no_prec": bool(args.no_prec),
    }


def numbers_match(a, b, tolerance=1e-12):
    try:
...
def legacy_part_matches(part, args, total):
    if int(part.get("orient_total", total)) != total:
        return False
    checks = [
        numbers_match(part.get("ka"), args.ka),
        part.get("shape") == args.shape,
        int(part.get("ntheta", -1)) == int(args.ntheta),
        int(part.get("alpha_avg", 1)) == int(args.alpha_avg),
        int(part.get("refinements", -1)) == int(args.subdiv),
        numbers_match(part.get("prism_aspect"), args.prism_aspect),
    ]
    part_ri = part.get("ri", [])
    checks.append(
        len(part_ri) == 2
        and numbers_match(part_ri[0], args.ri[0])
        and numbers_match(part_ri[1], args.ri[1])
    )
    if args.obj:
        checks.append(
            os.path.normpath(str(part.get("obj_file", ""))) == os.path.normpath(args.obj)
        )
    if args.quad is not None:
        checks.append(int(part.get("method", {}).get("quad_order", -1)) == int(args.quad))
    if args.system is not None:
        checks.append(part.get("requested_system") == args.system)
    if args.fmm_digits is not None:
        checks.append(int(part.get("fmm_digits", -1)) == int(args.fmm_digits))
    if args.gmres_tol is not None:
        checks.append(numbers_match(part.get("gmres_tol"), args.gmres_tol))
    if args.gmres_restart is not None:
        checks.append(int(part.get("gmres_restart", -1)) == int(args.gmres_restart))
    if args.max_leaf is not None:
        checks.append(int(part.get("max_leaf", -1)) == int(args.max_leaf))
    if args.no_prec:
        checks.append(part.get("preconditioner_enabled") is False)
    if int(part.get("gmres_nonconverged_systems", 0)) != 0:
        return False
    if int(part.get("gmres_numerical_breakdowns", 0)) != 0:
        return False
    if args.gmres_tol is not None:
        final_relres = float(part.get("gmres_max_final_relres", float("inf")))
        if final_relres > float(args.gmres_tol) * (1.0 + 1e-6):
            return False
    return all(checks)


def annotate_part_signature(path, part, signature):
    if part.get(RESUME_SIGNATURE_KEY) == signature:
        return
    part[RESUME_SIGNATURE_KEY] = signature
    tmp = path + ".resume.tmp"
    with open(tmp, "w") as stream:
        json.dump(part, stream, indent=2)
        stream.write("\n")
    os.replace(tmp, path)
...
def main():
    ...
    p.add_argument("--krylov",
                   choices=["gmres", "bicgstab", "bcgstab", "bicgstab-rr",
                            ...
                            "cgs", "cgs-rr", "cgs_rr", "gpu-gmres",
                            "gpu-adaptive", "gpu-native", "hybrid", "tfqmr",
                            "qmr2", "cocr", "block-cocg", "auto"],
                   default=None,
    ...
    resume_signature = build_resume_signature(args, orient_line_file)
    orient_lines = None
    if orient_line_file:
    ...
    def part_is_reusable(path, start, count, total):
        ...
        try:
            ...
            if not identity_matches:
                return False
            stored_signature = part.get(RESUME_SIGNATURE_KEY)
            if stored_signature is not None:
                return stored_signature == resume_signature
            return legacy_part_matches(part, args, total)
        except Exception:
            return False
    ...
    def annotate_completed_chunk(chunk):
        active = chunk.get("active")
        if active is None:
            paths = [os.path.join(args.work_dir, "part_%04d.json" % chunk["index"])]
        else:
            paths = [os.path.join(args.work_dir, "part_%04d.json" % idx) for idx in active]
        for path in paths:
            if not os.path.exists(path):
                raise SystemExit("completed chunk did not produce {}".format(path))
            with open(path, "r") as stream:
                part = json.load(stream)
            annotate_part_signature(path, part, resume_signature)
    ...
    while running:
        ...
        for item in list(running):
            ...
            annotate_completed_chunk(item["chunk"])
            finished.append(item)
    ...
    for path in part_paths:
        ...
        with open(path, "r") as stream:
            part = json.load(stream)
        annotate_part_signature(path, part, resume_signature)
        part_entries.append((path, part))
    parts = [part for _path, part in part_entries]
    if not parts:
