...
__global__ void tfqmr_even_update_pair_kernel(
    const double2* u1, const double2* u2,
    const double2* v1, const double2* v2,
    double2* un1, double2* un2,
    double2 alpha1, double2 alpha2, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    un1[i] = d_csub(u1[i], d_cmul(alpha1, v1[i]));
    un2[i] = d_csub(u2[i], d_cmul(alpha2, v2[i]));
}

__global__ void tfqmr_update_w_pair_kernel(
    double2* w1, double2* w2,
    const double2* uhat1, const double2* uhat2,
    double2 alpha1, double2 alpha2, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    w1[i] = d_csub(w1[i], d_cmul(alpha1, uhat1[i]));
    w2[i] = d_csub(w2[i], d_cmul(alpha2, uhat2[i]));
}

__global__ void tfqmr_update_dx_pair_kernel(
    const double2* u1, const double2* u2,
    double2* d1, double2* d2,
    double2* x1, double2* x2,
    double2 gamma1, double2 gamma2,
    double2 eta1, double2 eta2, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    d1[i] = d_cadd(u1[i], d_cmul(gamma1, d1[i]));
    d2[i] = d_cadd(u2[i], d_cmul(gamma2, d2[i]));
    x1[i] = d_cadd(x1[i], d_cmul(eta1, d1[i]));
    x2[i] = d_cadd(x2[i], d_cmul(eta2, d2[i]));
}

__global__ void tfqmr_odd_update_pair_kernel(
    double2* u1, double2* u2,
    double2* v1, double2* v2,
    const double2* w1, const double2* w2,
    const double2* uhat1, const double2* uhat2,
    double2 beta1, double2 beta2, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    u1[i] = d_cadd(w1[i], d_cmul(beta1, u1[i]));
    u2[i] = d_cadd(w2[i], d_cmul(beta2, u2[i]));
    double2 b1uh = d_cmul(beta1, uhat1[i]);
    double2 b2uh = d_cmul(beta2, uhat2[i]);
    v1[i] = d_cadd(b1uh, d_cmul(d_cmul(beta1, beta1), v1[i]));
    v2[i] = d_cadd(b2uh, d_cmul(d_cmul(beta2, beta2), v2[i]));
}

__global__ void qmr2_normalize_update_p_pair_kernel(
    double2* v1, double2* v2, double2* p1, double2* p2,
    double inv_ro1, double inv_ro2, double2 pcoef1, double2 pcoef2,
    int first, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    v1[i] = make_double2(v1[i].x * inv_ro1, v1[i].y * inv_ro1);
    v2[i] = make_double2(v2[i].x * inv_ro2, v2[i].y * inv_ro2);
    p1[i] = first ? v1[i] : d_cadd(v1[i], d_cmul(pcoef1, p1[i]));
    p2[i] = first ? v2[i] : d_cadd(v2[i], d_cmul(pcoef2, p2[i]));
}

__global__ void qmr2_form_v_pair_kernel(double2* v1, double2* v2,
    const double2* ap1, const double2* ap2, double2 beta1, double2 beta2, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    v1[i] = d_csub(ap1[i], d_cmul(beta1, v1[i]));
    v2[i] = d_csub(ap2[i], d_cmul(beta2, v2[i]));
}

__global__ void qmr2_update_dx_r_pair_kernel(
    const double2* v1, const double2* v2,
    const double2* p1, const double2* p2, double2* d1, double2* d2,
    double2* x1, double2* x2, double2* r1, double2* r2,
    double2 eta1, double2 eta2,
    double dcoef1, double dcoef2, double sabs21, double sabs22, int first, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    d1[i] = first ? d_cmul(eta1, p1[i])
                  : d_cadd(d_cmul(make_double2(dcoef1, 0.0), d1[i]), d_cmul(eta1, p1[i]));
    d2[i] = first ? d_cmul(eta2, p2[i])
                  : d_cadd(d_cmul(make_double2(dcoef2, 0.0), d2[i]), d_cmul(eta2, p2[i]));
    x1[i] = d_cadd(x1[i], d1[i]);
    x2[i] = d_cadd(x2[i], d2[i]);
    r1[i] = d_csub(make_double2(sabs21 * r1[i].x, sabs21 * r1[i].y), d_cmul(eta1, v1[i]));
    r2[i] = d_csub(make_double2(sabs22 * r2[i].x, sabs22 * r2[i].y), d_cmul(eta2, v2[i]));
}

__global__ void cocr_update_xr_pair_kernel(double2* x1, double2* x2,
    double2* r1, double2* r2, const double2* p1, const double2* p2,
    const double2* q1, const double2* q2, double2 a1, double2 a2, int n)
{
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i>=n) return;
    x1[i]=d_cadd(x1[i],d_cmul(a1,p1[i])); x2[i]=d_cadd(x2[i],d_cmul(a2,p2[i]));
    r1[i]=d_csub(r1[i],d_cmul(a1,q1[i])); r2[i]=d_csub(r2[i],d_cmul(a2,q2[i]));
}

__global__ void cocr_update_pq_pair_kernel(double2* p1, double2* p2,
    double2* q1, double2* q2, const double2* r1, const double2* r2,
    const double2* ar1, const double2* ar2, double2 b1, double2 b2, int n)
{
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i>=n) return;
    p1[i]=d_cadd(r1[i],d_cmul(b1,p1[i])); p2[i]=d_cadd(r2[i],d_cmul(b2,p2[i]));
    q1[i]=d_cadd(ar1[i],d_cmul(b1,q1[i])); q2[i]=d_cadd(ar2[i],d_cmul(b2,q2[i]));
}

__global__ void block_cocg_update_xr_kernel(double2* x1,double2* x2,double2* r1,double2* r2,
    const double2* p1,const double2* p2,const double2* q1,const double2* q2,
    double2 a11,double2 a12,double2 a21,double2 a22,int n)
{
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    x1[i]=d_cadd(x1[i],d_cadd(d_cmul(p1[i],a11),d_cmul(p2[i],a21)));
    x2[i]=d_cadd(x2[i],d_cadd(d_cmul(p1[i],a12),d_cmul(p2[i],a22)));
    r1[i]=d_csub(r1[i],d_cadd(d_cmul(q1[i],a11),d_cmul(q2[i],a21)));
    r2[i]=d_csub(r2[i],d_cadd(d_cmul(q1[i],a12),d_cmul(q2[i],a22)));
}

__global__ void block_cocg_update_p_kernel(double2* p1,double2* p2,const double2* r1,const double2* r2,
    double2 b11,double2 b12,double2 b21,double2 b22,int n)
{
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    double2 o1=p1[i],o2=p2[i];
    p1[i]=d_cadd(r1[i],d_cadd(d_cmul(o1,b11),d_cmul(o2,b21)));
    p2[i]=d_cadd(r2[i],d_cadd(d_cmul(o1,b12),d_cmul(o2,b22)));
}
...
__global__ void qmr2_dot_pair_reduce_kernel(const double2* v1, const double2* w1,
                                            const double2* v2, const double2* w2,
                                            double* pr1, double* pi1,
                                            double* pr2, double* pi2, int n)
{
    extern __shared__ double sh[];
    double* r1 = sh; double* i1 = sh + blockDim.x;
    double* r2 = sh + 2 * blockDim.x; double* i2 = sh + 3 * blockDim.x;
    int tid = threadIdx.x, idx = blockIdx.x * blockDim.x + tid;
    double2 z1 = make_double2(0.0, 0.0), z2 = make_double2(0.0, 0.0);
    if (idx < n) {
        z1 = d_cmul(v1[idx], w1[idx]);
        z2 = d_cmul(v2[idx], w2[idx]);
    }
    r1[tid]=z1.x; i1[tid]=z1.y; r2[tid]=z2.x; i2[tid]=z2.y;
    __syncthreads();
    for (int stride=blockDim.x/2; stride>0; stride>>=1) {
        if (tid < stride) {
            r1[tid]+=r1[tid+stride]; i1[tid]+=i1[tid+stride];
            r2[tid]+=r2[tid+stride]; i2[tid]+=i2[tid+stride];
        }
        __syncthreads();
    }
    if (tid==0) { pr1[blockIdx.x]=r1[0]; pi1[blockIdx.x]=i1[0];
                  pr2[blockIdx.x]=r2[0]; pi2[blockIdx.x]=i2[0]; }
}
...
static void gpu_dot_transpose_pair(double2* d_v1, double2* d_w1,
                                   double2* d_v2, double2* d_w2, int n,
                                   double* d_pr1, double* d_pi1,
                                   double* d_pr2, double* d_pi2,
                                   double* d_reduce4,
                                   cdouble& h1, cdouble& h2)
{
    const int block = 256;
    int grid = (n + block - 1) / block;
    qmr2_dot_pair_reduce_kernel<<<grid, block, 4 * block * sizeof(double)>>>(
        d_v1, d_w1, d_v2, d_w2, d_pr1, d_pi1, d_pr2, d_pi2, n);
    CUDA_CHECK(cudaGetLastError());
    gmres_reduce4_final_kernel<<<1, block, 4 * block * sizeof(double)>>>(
        d_pr1, d_pi1, d_pr2, d_pi2,
    ...
    CUDA_CHECK(cudaGetLastError());
    double h[4];
    CUDA_CHECK(cudaMemcpy(h, d_reduce4, sizeof(h), cudaMemcpyDeviceToHost));
    h1 = cdouble(h[0], h[1]);
    h2 = cdouble(h[2], h[3]);
}
...
static bool solve2x2_right(const cdouble H[4],const cdouble G[4],cdouble X[4])
{
    cdouble det=H[0]*H[3]-H[1]*H[2];if(std::abs(det)<1e-20)return false;
    cdouble i00=H[3]/det,i01=-H[1]/det,i10=-H[2]/det,i11=H[0]/det;
    X[0]=i00*G[0]+i01*G[2];X[1]=i00*G[1]+i01*G[3];
    X[2]=i10*G[0]+i11*G[2];X[3]=i10*G[1]+i11*G[3];return true;
}

int block_cocg_solve_paired_device_ws(BemFmmOperator& op,const cdouble* b1,const cdouble* b2,
    cdouble* x1,cdouble* x2,int restart,double tol,int maxiter,bool verbose,GmresPairedWorkspace& ws)
{
    const int n=op.system_size,block=256,grid=(n+block-1)/block,max_steps=std::max(1,restart)*std::max(1,maxiter);
    const size_t bytes=(size_t)n*sizeof(double2);const int true_every=std::max(1,bem_env_int("BEM_BLOCK_COCG_TRUE_EVERY",10));
    BicgstabDeviceBufferCache& c=bicgstab_device_buffer_cache();c.ensure(n,grid);
    double2 *db1=c.d_b1,*db2=c.d_b2,*dx1=c.d_x1,*dx2=c.d_x2,*r1=c.d_r1,*r2=c.d_r2;
    double2 *p1=c.d_p1,*p2=c.d_p2,*q1=c.d_v1,*q2=c.d_v2,*best1=c.d_best_x1,*best2=c.d_best_x2;
    CUDA_CHECK(cudaMemcpy(db1,b1,bytes,cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(db2,b2,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dx1,x1,bytes,cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(dx2,x2,bytes,cudaMemcpyHostToDevice));
    double bn1,bn2;gpu_norm_pair(db1,db2,n,c.d_np1,c.d_np2,c.d_reduce2,bn1,bn2);if(bn1<1e-30)bn1=1;if(bn2<1e-30)bn2=1;
    op.matvec_batch2_device(dx1,dx2,q1,q2);int mv=1;double rn1,rn2;
    gpu_b_minus_ax_norm_pair(db1,db2,q1,q2,r1,r2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);
    CUDA_CHECK(cudaMemcpy(p1,r1,bytes,cudaMemcpyDeviceToDevice));CUDA_CHECK(cudaMemcpy(p2,r2,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice));CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));
    auto gram=[&](double2* v1,double2* v2,double2* w1,double2* w2,cdouble M[4]){
        gpu_dot_transpose_pair(v1,w1,v2,w2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,M[0],M[3]);
        gpu_dot_transpose_pair(v1,w2,v2,w1,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,M[1],M[2]);
    };
    cdouble G[4];gram(r1,r2,r1,r2,G);double bestrel1=rn1/bn1,bestrel2=rn2/bn2;bool conv1=bestrel1<tol,conv2=bestrel2<tol,breakdown=false;
    for(int it=1;it<=max_steps&&!(conv1&&conv2);++it){
        op.matvec_batch2_device(p1,p2,q1,q2);++mv;cdouble H[4],A[4];gram(p1,p2,q1,q2,H);
        if(!solve2x2_right(H,G,A)){breakdown=true;break;}
        block_cocg_update_xr_kernel<<<grid,block>>>(dx1,dx2,r1,r2,p1,p2,q1,q2,to_double2(A[0]),to_double2(A[1]),to_double2(A[2]),to_double2(A[3]),n);CUDA_CHECK(cudaGetLastError());
        cdouble Gn[4],B[4];gram(r1,r2,r1,r2,Gn);if(!solve2x2_right(G,Gn,B)){breakdown=true;break;}
        block_cocg_update_p_kernel<<<grid,block>>>(p1,p2,r1,r2,to_double2(B[0]),to_double2(B[1]),to_double2(B[2]),to_double2(B[3]),n);CUDA_CHECK(cudaGetLastError());
        for(int k=0;k<4;k++)G[k]=Gn[k];
        if(it%true_every==0||it==max_steps){op.matvec_batch2_device(dx1,dx2,q1,q2);++mv;gpu_b_minus_ax_norm_pair(db1,db2,q1,q2,c.d_s1,c.d_s2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);double re1=rn1/bn1,re2=rn2/bn2;if(re1<bestrel1){bestrel1=re1;CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice));}if(re2<bestrel2){bestrel2=re2;CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));}conv1=re1<tol;conv2=re2<tol;if(verbose)printf("  [Block-COCG-GPU] iter=%d true res1=%.2e res2=%.2e\n",it,re1,re2);}
    }
    CUDA_CHECK(cudaMemcpy(x1,best1,bytes,cudaMemcpyDeviceToHost));CUDA_CHECK(cudaMemcpy(x2,best2,bytes,cudaMemcpyDeviceToHost));ws.final_relres1=bestrel1;ws.final_relres2=bestrel2;ws.converged1=bestrel1<tol;ws.converged2=bestrel2<tol;ws.numerical_breakdown=breakdown;ws.reached_max_cycles=!(ws.converged1&&ws.converged2)&&!breakdown;ws.restored_best_iterate=true;if(verbose){printf("  [Block-COCG-GPU] done: matvecs=%d best1=%.2e best2=%.2e%s\n",mv,bestrel1,bestrel2,breakdown?" breakdown":"");fflush(stdout);}return mv;
}

int cocr_solve_paired_device_ws(BemFmmOperator& op,
                                const cdouble* b1,const cdouble* b2,
                                cdouble* x1,cdouble* x2,int restart,double tol,
                                int maxiter,bool verbose,GmresPairedWorkspace& ws,
                                NearFieldPrecond* sym_precond)
{
    const int n=op.system_size,block=256,grid=(n+block-1)/block;
    const size_t bytes=(size_t)n*sizeof(double2);
    const int max_steps=std::max(1,restart)*std::max(1,maxiter);
    const int true_every=std::max(1,bem_env_int("BEM_COCR_TRUE_EVERY",10));
    BicgstabDeviceBufferCache& c=bicgstab_device_buffer_cache(); c.ensure(n,grid);
    double2 *db1=c.d_b1,*db2=c.d_b2,*dx1=c.d_x1,*dx2=c.d_x2;
    double2 *best1=c.d_best_x1,*best2=c.d_best_x2,*r1=c.d_r1,*r2=c.d_r2;
    double2 *p1=c.d_p1,*p2=c.d_p2,*q1=c.d_v1,*q2=c.d_v2,*ar1=c.d_s1,*ar2=c.d_s2;
    double2 *tmpx1=c.d_rh1,*tmpx2=c.d_rh2,*tmpy1=c.d_t1,*tmpy2=c.d_t2;
    CUDA_CHECK(cudaMemcpy(db1,b1,bytes,cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemcpy(db2,b2,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dx1,x1,bytes,cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemcpy(dx2,x2,bytes,cudaMemcpyHostToDevice));
    const bool sym_prec=sym_precond&&sym_precond->symmetric_device_apply_available();
    auto apply_pair=[&](double2* in1,double2* in2,double2* out1,double2* out2){
        if(!sym_prec){op.matvec_batch2_device(in1,in2,out1,out2);return;}
        sym_precond->apply_symmetric_device(in1,tmpx1);sym_precond->apply_symmetric_device(in2,tmpx2);
        op.matvec_batch2_device(tmpx1,tmpx2,tmpy1,tmpy2);
        sym_precond->apply_symmetric_device(tmpy1,out1);sym_precond->apply_symmetric_device(tmpy2,out2);
    };
    if(sym_prec){
        CUDA_CHECK(cudaMemset(dx1,0,bytes));CUDA_CHECK(cudaMemset(dx2,0,bytes));
        sym_precond->apply_symmetric_device(db1,tmpx1);sym_precond->apply_symmetric_device(db2,tmpx2);
        CUDA_CHECK(cudaMemcpy(db1,tmpx1,bytes,cudaMemcpyDeviceToDevice));CUDA_CHECK(cudaMemcpy(db2,tmpx2,bytes,cudaMemcpyDeviceToDevice));
    }
    double bn1,bn2; gpu_norm_pair(db1,db2,n,c.d_np1,c.d_np2,c.d_reduce2,bn1,bn2); if(bn1<1e-30)bn1=1;if(bn2<1e-30)bn2=1;
    apply_pair(dx1,dx2,q1,q2); int matvecs=1;
    double rn1,rn2; gpu_b_minus_ax_norm_pair(db1,db2,q1,q2,r1,r2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);
    apply_pair(r1,r2,ar1,ar2); ++matvecs;
    CUDA_CHECK(cudaMemcpy(p1,r1,bytes,cudaMemcpyDeviceToDevice)); CUDA_CHECK(cudaMemcpy(p2,r2,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(q1,ar1,bytes,cudaMemcpyDeviceToDevice)); CUDA_CHECK(cudaMemcpy(q2,ar2,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice)); CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));
    cdouble rho1,rho2; gpu_dot_transpose_pair(r1,ar1,r2,ar2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,rho1,rho2);
    double bestrel1=rn1/bn1,bestrel2=rn2/bn2; bool breakdown=false,conv1=bestrel1<tol,conv2=bestrel2<tol;
    if(verbose){printf("  [COCR-paired-GPU] start: res1=%.2e res2=%.2e\n",bestrel1,bestrel2);fflush(stdout);}
    int it=0;
    for(it=1;it<=max_steps&&!(conv1&&conv2);++it){
        cdouble qq1,qq2; gpu_dot_transpose_pair(q1,q1,q2,q2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,qq1,qq2);
        if(std::abs(qq1)<1e-14||std::abs(qq2)<1e-14||std::abs(rho1)<1e-14||std::abs(rho2)<1e-14){breakdown=true;break;}
        cdouble a1=rho1/qq1,a2=rho2/qq2;
        cocr_update_xr_pair_kernel<<<grid,block>>>(dx1,dx2,r1,r2,p1,p2,q1,q2,to_double2(a1),to_double2(a2),n); CUDA_CHECK(cudaGetLastError());
        apply_pair(r1,r2,ar1,ar2);++matvecs;
        cdouble rhon1,rhon2;gpu_dot_transpose_pair(r1,ar1,r2,ar2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,rhon1,rhon2);
        cdouble be1=rhon1/rho1,be2=rhon2/rho2;rho1=rhon1;rho2=rhon2;
        cocr_update_pq_pair_kernel<<<grid,block>>>(p1,p2,q1,q2,r1,r2,ar1,ar2,to_double2(be1),to_double2(be2),n);CUDA_CHECK(cudaGetLastError());
        if(it%true_every==0||it==max_steps){
            apply_pair(dx1,dx2,ar1,ar2);++matvecs;
            gpu_b_minus_ax_norm_pair(db1,db2,ar1,ar2,tmpy1,tmpy2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);
            double rel1=rn1/bn1,rel2=rn2/bn2;
            if(rel1<bestrel1){bestrel1=rel1;CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice));}
            if(rel2<bestrel2){bestrel2=rel2;CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));}
            conv1=rel1<tol;conv2=rel2<tol;if(verbose)printf("  [COCR-paired-GPU] iter=%d true res1=%.2e res2=%.2e\n",it,rel1,rel2);
        }
    }
    if(sym_prec){sym_precond->apply_symmetric_device(best1,tmpx1);sym_precond->apply_symmetric_device(best2,tmpx2);best1=tmpx1;best2=tmpx2;}
    CUDA_CHECK(cudaMemcpy(x1,best1,bytes,cudaMemcpyDeviceToHost));CUDA_CHECK(cudaMemcpy(x2,best2,bytes,cudaMemcpyDeviceToHost));
    ws.final_relres1=bestrel1;ws.final_relres2=bestrel2;ws.converged1=bestrel1<tol;ws.converged2=bestrel2<tol;ws.numerical_breakdown=breakdown;ws.reached_max_cycles=!(ws.converged1&&ws.converged2)&&!breakdown;ws.restored_best_iterate=true;
    if(verbose){printf("  [COCR-paired-GPU] done: matvecs=%d best1=%.2e best2=%.2e%s\n",matvecs,bestrel1,bestrel2,breakdown?" breakdown":"");fflush(stdout);}return matvecs;
}
                                ...
                                bool verbose, GmresPairedWorkspace& ws)
{
    ...
    const size_t bytes = (size_t)n * sizeof(double2);
    const int max_steps = std::max(1, restart) * std::max(1, maxiter);
    const int true_every = std::max(1, bem_env_int("BEM_QMR2_TRUE_EVERY", 10));
    BicgstabDeviceBufferCache& c = bicgstab_device_buffer_cache();
    c.ensure(n, grid);
    double2 *db1=c.d_b1,*db2=c.d_b2,*dx1=c.d_x1,*dx2=c.d_x2;
    double2 *best1=c.d_best_x1,*best2=c.d_best_x2,*r1=c.d_r1,*r2=c.d_r2;
    double2 *v1=c.d_v1,*v2=c.d_v2,*p1=c.d_p1,*p2=c.d_p2;
    double2 *ap1=c.d_s1,*ap2=c.d_s2,*d1=c.d_t1,*d2=c.d_t2;
    CUDA_CHECK(cudaMemcpy(db1,b1,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(db2,b2,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dx1,x1,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dx2,x2,bytes,cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemset(d1,0,bytes)); CUDA_CHECK(cudaMemset(d2,0,bytes));
    double bn1,bn2; gpu_norm_pair(db1,db2,n,c.d_np1,c.d_np2,c.d_reduce2,bn1,bn2);
    if (bn1<1e-30) bn1=1; if (bn2<1e-30) bn2=1;
    op.matvec_batch2_device(dx1,dx2,ap1,ap2);
    int matvecs=1;
    double rn1,rn2;
    gpu_b_minus_ax_norm_pair(db1,db2,ap1,ap2,r1,r2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);
    CUDA_CHECK(cudaMemcpy(v1,r1,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(v2,r2,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));
    double ro1=rn1,ro2=rn2,cold1=1,cold2=1,thold1=0,thold2=0;
    double bestrel1=rn1/bn1,bestrel2=rn2/bn2;
    cdouble eps1(1),eps2(1),eta1(-1),eta2(-1);
    bool breakdown=false, conv1=bestrel1<tol,conv2=bestrel2<tol;
    int iter=0;
    if (verbose) printf("  [QMR2-paired-GPU] start: res1=%.2e res2=%.2e\n",bestrel1,bestrel2);
    for (iter=1; iter<=max_steps && !(conv1&&conv2); ++iter) {
        if (ro1<1e-300 || ro2<1e-300) { breakdown=true; break; }
        cdouble delta1,delta2;
        cdouble pc1 = iter==1 ? cdouble(0) : -ro1*cdouble(1)*cdouble(1)/eps1;
        cdouble pc2 = iter==1 ? cdouble(0) : -ro2*cdouble(1)*cdouble(1)/eps2;
        // delta is needed in p coefficient, so normalize first, reduce, then update p.
        gmres_scale_one_kernel<<<grid,block>>>(ap1,v1,1.0/ro1,n);
        gmres_scale_one_kernel<<<grid,block>>>(ap2,v2,1.0/ro2,n);
        CUDA_CHECK(cudaGetLastError());
        gpu_dot_transpose_pair(ap1,ap1,ap2,ap2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,delta1,delta2);
        if (std::abs(delta1)<1e-14 || std::abs(delta2)<1e-14) { breakdown=true; break; }
        pc1 = iter==1 ? cdouble(0) : -ro1*delta1/eps1;
        pc2 = iter==1 ? cdouble(0) : -ro2*delta2/eps2;
        // ap currently stores normalized v; copy back and form p.
        CUDA_CHECK(cudaMemcpy(v1,ap1,bytes,cudaMemcpyDeviceToDevice));
        CUDA_CHECK(cudaMemcpy(v2,ap2,bytes,cudaMemcpyDeviceToDevice));
        qmr2_normalize_update_p_pair_kernel<<<grid,block>>>(v1,v2,p1,p2,1.0,1.0,
            to_double2(pc1),to_double2(pc2),iter==1,n);
        CUDA_CHECK(cudaGetLastError());
        op.matvec_batch2_device(p1,p2,ap1,ap2); ++matvecs;
        gpu_dot_transpose_pair(p1,ap1,p2,ap2,n,c.d_pr1,c.d_pi1,c.d_pr2,c.d_pi2,c.d_reduce4,eps1,eps2);
        if (std::abs(eps1)<1e-14 || std::abs(eps2)<1e-14) { breakdown=true; break; }
        cdouble beta1=eps1/delta1,beta2=eps2/delta2;
        // Form v~ temporarily to obtain its Euclidean norm.
        qmr2_form_v_pair_kernel<<<grid,block>>>(v1,v2,ap1,ap2,
                                               to_double2(beta1),to_double2(beta2),n);
        CUDA_CHECK(cudaGetLastError());
        double ronew1,ronew2; gpu_norm_pair(v1,v2,n,c.d_np1,c.d_np2,c.d_reduce2,ronew1,ronew2);
        double th1=ronew1/(cold1*std::abs(beta1)), th2=ronew2/(cold2*std::abs(beta2));
        double cn1=1/std::sqrt(1+th1*th1),cn2=1/std::sqrt(1+th2*th2);
        double sa1=th1*th1/(1+th1*th1),sa2=th2*th2/(1+th2*th2);
        eta1=-ro1*(cn1/cold1)*(cn1/cold1)*eta1/beta1;
        eta2=-ro2*(cn2/cold2)*(cn2/cold2)*eta2/beta2;
        qmr2_update_dx_r_pair_kernel<<<grid,block>>>(v1,v2,p1,p2,d1,d2,dx1,dx2,r1,r2,
            to_double2(eta1),to_double2(eta2),
            (thold1*cn1)*(thold1*cn1),(thold2*cn2)*(thold2*cn2),sa1,sa2,iter==1,n);
        CUDA_CHECK(cudaGetLastError());
        ro1=ronew1;ro2=ronew2;thold1=th1;thold2=th2;cold1=cn1;cold2=cn2;
        if (iter%true_every==0 || iter==max_steps) {
            op.matvec_batch2_device(dx1,dx2,ap1,ap2); ++matvecs;
            gpu_b_minus_ax_norm_pair(db1,db2,ap1,ap2,r1,r2,n,c.d_np1,c.d_np2,c.d_reduce2,rn1,rn2);
            double rel1=rn1/bn1,rel2=rn2/bn2;
            if (rel1<bestrel1) {bestrel1=rel1;CUDA_CHECK(cudaMemcpy(best1,dx1,bytes,cudaMemcpyDeviceToDevice));}
            if (rel2<bestrel2) {bestrel2=rel2;CUDA_CHECK(cudaMemcpy(best2,dx2,bytes,cudaMemcpyDeviceToDevice));}
            conv1=rel1<tol;conv2=rel2<tol;
            if (verbose) printf("  [QMR2-paired-GPU] iter=%d true res1=%.2e res2=%.2e\n",iter,rel1,rel2);
        }
    }
    CUDA_CHECK(cudaMemcpy(x1,best1,bytes,cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(x2,best2,bytes,cudaMemcpyDeviceToHost));
    ws.final_relres1=bestrel1;ws.final_relres2=bestrel2;ws.converged1=bestrel1<tol;ws.converged2=bestrel2<tol;
    ws.numerical_breakdown=breakdown;ws.reached_max_cycles=!(ws.converged1&&ws.converged2)&&!breakdown;
    ws.restored_best_iterate=true;
    if (verbose) { printf("  [QMR2-paired-GPU] done: matvecs=%d best1=%.2e best2=%.2e%s\n",matvecs,bestrel1,bestrel2,breakdown?" breakdown":""); fflush(stdout); }
    return matvecs;
}
                                 ...
                                 bool verbose, GmresPairedWorkspace& ws)
{
    ...
    const size_t vec_bytes = (size_t)n * sizeof(double2);
    const int max_steps = std::max(1, restart) * std::max(1, maxiter);
    const int true_every = std::max(2, bem_env_int("BEM_TFQMR_TRUE_EVERY", 20));

    ws = GmresPairedWorkspace();
    BicgstabDeviceBufferCache& cache = bicgstab_device_buffer_cache();
    cache.ensure(n, red_grid);
    double2 *d_b1 = cache.d_b1, *d_b2 = cache.d_b2;
    double2 *d_x1 = cache.d_x1, *d_x2 = cache.d_x2;
    ...
    double2 *d_r1 = cache.d_r1, *d_r2 = cache.d_r2;
    double2 *d_rh1 = cache.d_rh1, *d_rh2 = cache.d_rh2;
    double2 *d_u1 = cache.d_p1, *d_u2 = cache.d_p2;
    double2 *d_v1 = cache.d_v1, *d_v2 = cache.d_v2;
    double2 *d_w1 = cache.d_s1, *d_w2 = cache.d_s2;
    double2 *d_uh1 = cache.d_t1, *d_uh2 = cache.d_t2;
    double *d_np1 = cache.d_np1, *d_np2 = cache.d_np2;
    double *d_pr1 = cache.d_pr1, *d_pi1 = cache.d_pi1;
    double *d_pr2 = cache.d_pr2, *d_pi2 = cache.d_pi2;
    double *d_reduce2 = cache.d_reduce2, *d_reduce4 = cache.d_reduce4;
    double2 *d_d1 = nullptr, *d_d2 = nullptr, *d_un1 = nullptr, *d_un2 = nullptr;
    CUDA_CHECK(cudaMalloc(&d_d1, vec_bytes));
    CUDA_CHECK(cudaMalloc(&d_d2, vec_bytes));
    CUDA_CHECK(cudaMalloc(&d_un1, vec_bytes));
    CUDA_CHECK(cudaMalloc(&d_un2, vec_bytes));

    auto release_local = [&]() {
        cudaFree(d_d1); cudaFree(d_d2); cudaFree(d_un1); cudaFree(d_un2);
    };
    ...
    CUDA_CHECK(cudaMemcpy(d_best_x1, d_x1, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_best_x2, d_x2, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemset(d_d1, 0, vec_bytes));
    CUDA_CHECK(cudaMemset(d_d2, 0, vec_bytes));
    ...
    double xnorm1 = 0.0, xnorm2 = 0.0;
    gpu_norm_pair(d_x1, d_x2, n, d_np1, d_np2, d_reduce2, xnorm1, xnorm2);
    if (xnorm1 > 1e-30 || xnorm2 > 1e-30) {
        op.matvec_batch2_device(d_x1, d_x2, d_un1, d_un2);
        matvecs++;
        double rn1 = 0.0, rn2 = 0.0;
        gpu_b_minus_ax_norm_pair(d_b1, d_b2, d_un1, d_un2, d_r1, d_r2, n,
                                 d_np1, d_np2, d_reduce2, rn1, rn2);
        if (std::max(rn1 / bnorm1, rn2 / bnorm2) >
            bem_env_double("BEM_GMRES_WARM_MAX_REL", 1.05)) {
            CUDA_CHECK(cudaMemset(d_x1, 0, vec_bytes));
            CUDA_CHECK(cudaMemset(d_x2, 0, vec_bytes));
            CUDA_CHECK(cudaMemcpy(d_r1, d_b1, vec_bytes, cudaMemcpyDeviceToDevice));
            CUDA_CHECK(cudaMemcpy(d_r2, d_b2, vec_bytes, cudaMemcpyDeviceToDevice));
        }
    } else {
        CUDA_CHECK(cudaMemcpy(d_r1, d_b1, vec_bytes, cudaMemcpyDeviceToDevice));
        CUDA_CHECK(cudaMemcpy(d_r2, d_b2, vec_bytes, cudaMemcpyDeviceToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(d_rh1, d_r1, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_rh2, d_r2, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_u1, d_r1, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_u2, d_r2, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_w1, d_r1, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_w2, d_r2, vec_bytes, cudaMemcpyDeviceToDevice));
    op.matvec_batch2_device(d_u1, d_u2, d_v1, d_v2);
    matvecs++;
    CUDA_CHECK(cudaMemcpy(d_uh1, d_v1, vec_bytes, cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy(d_uh2, d_v2, vec_bytes, cudaMemcpyDeviceToDevice));

    double tau1 = 0.0, tau2 = 0.0;
    gpu_norm_pair(d_r1, d_r2, n, d_np1, d_np2, d_reduce2, tau1, tau2);
    cdouble rho1, rho2;
    gpu_dot_pair(d_rh1, d_r1, d_rh2, d_r2, n,
                 d_pr1, d_pi1, d_pr2, d_pi2, d_reduce4, rho1, rho2);
    cdouble rho_last1 = rho1, rho_last2 = rho2;
    cdouble alpha1(1.0), alpha2(1.0), eta1(0.0), eta2(0.0);
    double theta1 = 0.0, theta2 = 0.0;
    double rel1 = tau1 / bnorm1, rel2 = tau2 / bnorm2;
    double best_rel1 = rel1, best_rel2 = rel2;
    bool conv1 = rel1 < tol, conv2 = rel2 < tol;
    bool breakdown = false;

    if (verbose) {
        printf("  [TFQMR-paired-GPU] start: res1=%.2e res2=%.2e true_every=%d\n",
               rel1, rel2, true_every);
        fflush(stdout);
    }
    ...
    for (; iter < max_steps && !(conv1 && conv2); ++iter) {
        ...
        if (even) {
            cdouble rv1, rv2;
            gpu_dot_pair(d_rh1, d_v1, d_rh2, d_v2, n,
                         d_pr1, d_pi1, d_pr2, d_pi2, d_reduce4, rv1, rv2);
            if ((!conv1 && !usable_divisor(rv1)) || (!conv2 && !usable_divisor(rv2))) {
                breakdown = true;
                break;
            }
            if (!conv1) alpha1 = rho1 / rv1;
            if (!conv2) alpha2 = rho2 / rv2;
            tfqmr_even_update_pair_kernel<<<grid, block>>>(
                d_u1, d_u2, d_v1, d_v2, d_un1, d_un2,
                to_double2(alpha1), to_double2(alpha2), n);
            CUDA_CHECK(cudaGetLastError());
        }

        tfqmr_update_w_pair_kernel<<<grid, block>>>(
            d_w1, d_w2, d_uh1, d_uh2, to_double2(alpha1), to_double2(alpha2), n);
        CUDA_CHECK(cudaGetLastError());
        double wnorm1 = 0.0, wnorm2 = 0.0;
        gpu_norm_pair(d_w1, d_w2, n, d_np1, d_np2, d_reduce2, wnorm1, wnorm2);
        if ((!conv1 && tau1 <= 1e-300) || (!conv2 && tau2 <= 1e-300) ||
            (!conv1 && !usable_divisor(alpha1)) || (!conv2 && !usable_divisor(alpha2))) {
            breakdown = true;
            break;
        }
        double old_theta1 = theta1, old_theta2 = theta2;
        cdouble old_eta1 = eta1, old_eta2 = eta2;
        theta1 = conv1 ? 0.0 : wnorm1 / tau1;
        theta2 = conv2 ? 0.0 : wnorm2 / tau2;
        double c1 = 1.0 / std::sqrt(1.0 + theta1 * theta1);
        double c2 = 1.0 / std::sqrt(1.0 + theta2 * theta2);
        tau1 *= theta1 * c1;
        tau2 *= theta2 * c2;
        eta1 = c1 * c1 * alpha1;
        eta2 = c2 * c2 * alpha2;
        cdouble gamma1 = (old_theta1 * old_theta1) * old_eta1 / alpha1;
        cdouble gamma2 = (old_theta2 * old_theta2) * old_eta2 / alpha2;
        tfqmr_update_dx_pair_kernel<<<grid, block>>>(
            d_u1, d_u2, d_d1, d_d2, d_x1, d_x2,
            to_double2(gamma1), to_double2(gamma2),
            to_double2(eta1), to_double2(eta2), n);
        CUDA_CHECK(cudaGetLastError());

        const double bound1 = tau1 * std::sqrt((double)iter + 1.0) / bnorm1;
        const double bound2 = tau2 * std::sqrt((double)iter + 1.0) / bnorm2;
        const bool check_true = ((iter + 1) % true_every == 0) ||
                                std::max(bound1, bound2) < tol;
        if (check_true) {
            op.matvec_batch2_device(d_x1, d_x2, d_un1, d_un2);
            matvecs++;
            double rn1 = 0.0, rn2 = 0.0;
            ...
            if (verbose) {
                printf("    TFQMR-GPU iter %d: bound1=%.2e bound2=%.2e true1=%.2e true2=%.2e matvecs=%d\n",
                       iter + 1, bound1, bound2, rel1, rel2, matvecs);
                fflush(stdout);
            }
        }
        ...
        if (even) {
            op.matvec_batch2_device(d_un1, d_un2, d_uh1, d_uh2);
            matvecs++;
            CUDA_CHECK(cudaMemcpy(d_u1, d_un1, vec_bytes, cudaMemcpyDeviceToDevice));
            CUDA_CHECK(cudaMemcpy(d_u2, d_un2, vec_bytes, cudaMemcpyDeviceToDevice));
            rho_last1 = rho1;
            rho_last2 = rho2;
        } else {
            gpu_dot_pair(d_rh1, d_w1, d_rh2, d_w2, n,
                         d_pr1, d_pi1, d_pr2, d_pi2, d_reduce4, rho1, rho2);
            if ((!conv1 && !usable_divisor(rho_last1)) ||
                (!conv2 && !usable_divisor(rho_last2))) {
                breakdown = true;
                break;
            }
            cdouble beta1 = conv1 ? cdouble(0.0) : rho1 / rho_last1;
            cdouble beta2 = conv2 ? cdouble(0.0) : rho2 / rho_last2;
            tfqmr_odd_update_pair_kernel<<<grid, block>>>(
                d_u1, d_u2, d_v1, d_v2, d_w1, d_w2, d_uh1, d_uh2,
                to_double2(beta1), to_double2(beta2), n);
            CUDA_CHECK(cudaGetLastError());
            op.matvec_batch2_device(d_u1, d_u2, d_un1, d_un2);
            matvecs++;
            CUDA_CHECK(cudaMemcpy(d_uh1, d_un1, vec_bytes, cudaMemcpyDeviceToDevice));
            CUDA_CHECK(cudaMemcpy(d_uh2, d_un2, vec_bytes, cudaMemcpyDeviceToDevice));
            gmres_axpy_one_kernel<<<grid, block>>>(d_v1, make_double2(1.0, 0.0), d_uh1, n);
            gmres_axpy_one_kernel<<<grid, block>>>(d_v2, make_double2(1.0, 0.0), d_uh2, n);
            CUDA_CHECK(cudaGetLastError());
        }
    }
    ...
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<double2*>(x1), d_best_x1, vec_bytes, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<double2*>(x2), d_best_x2, vec_bytes, cudaMemcpyDeviceToHost));
    ws.final_relres1 = best_rel1;
    ws.final_relres2 = best_rel2;
    ws.converged1 = best_rel1 < tol;
    ws.converged2 = best_rel2 < tol;
    ws.numerical_breakdown = breakdown;
    ws.reached_max_cycles = !(ws.converged1 && ws.converged2) && !breakdown;
    ws.restored_best_iterate = true;
    release_local();
    if (verbose) {
        printf("  [TFQMR-paired-GPU] %s, %d iterations, %d matvec evaluations, res1=%.2e res2=%.2e\n",
               (ws.converged1 && ws.converged2) ? "Both converged" : "NOT fully converged",
               iter + 1, matvecs, best_rel1, best_rel2);
        fflush(stdout);
    }
    return matvecs;
}
                          ...
                          GmresPairedWorkspace& ws)
{
    ...
    bool tfqmr_requested = krylov && strcmp(krylov, "tfqmr") == 0;
    bool qmr2_requested = krylov && strcmp(krylov, "qmr2") == 0;
    bool cocr_requested = krylov && strcmp(krylov, "cocr") == 0;
    bool block_cocg_requested = krylov && strcmp(krylov, "block-cocg") == 0;
    ...
    if (precond==nullptr&&block_cocg_requested&&op.device_matvec_available()) {
        int matvecs=block_cocg_solve_paired_device_ws(op,b1,b2,x1,x2,restart,tol,maxiter,verbose,ws);
        if(!(ws.converged1&&ws.converged2)&&bem_env_flag_enabled("BEM_BLOCK_COCG_FALLBACK_GMRES",true))
            return continue_with_device_gmres(matvecs,"Block-COCG-GPU");
        return matvecs;
    }

    if (cocr_requested) {
        if (op.device_matvec_available()) {
            NearFieldPrecond* symp=(precond&&bem_env_flag_enabled("BEM_COCR_SYM_PREC"))?precond:nullptr;
            int matvecs=cocr_solve_paired_device_ws(op,b1,b2,x1,x2,restart,tol,maxiter,verbose,ws,symp);
            if(!(ws.converged1&&ws.converged2)&&bem_env_flag_enabled("BEM_COCR_FALLBACK_GMRES",true))
                return continue_with_device_gmres(matvecs,"COCR-paired-GPU");
            return matvecs;
        }
    }

    if (precond == nullptr && qmr2_requested) {
        if (op.device_matvec_available()) {
            int matvecs = qmr2_solve_paired_device_ws(op, b1, b2, x1, x2,
                                                      restart, tol, maxiter,
                                                      verbose, ws);
            if (!(ws.converged1 && ws.converged2) &&
                bem_env_flag_enabled("BEM_QMR2_FALLBACK_GMRES", true))
                return continue_with_device_gmres(matvecs, "QMR2-paired-GPU");
            return matvecs;
        }
        if (verbose) printf("  [QMR2-paired-GPU] device matvec unavailable; using GMRES\n");
    }

    if (precond == nullptr && tfqmr_requested) {
        if (op.device_matvec_available()) {
            int matvecs = tfqmr_solve_paired_device_ws(op, b1, b2, x1, x2,
                                                       restart, tol, maxiter,
                                                       verbose, ws);
            if (!(ws.converged1 && ws.converged2) &&
                bem_env_flag_enabled("BEM_TFQMR_FALLBACK_GMRES", true)) {
                return continue_with_device_gmres(matvecs, "TFQMR-paired-GPU");
            }
            return matvecs;
        }
        if (verbose) {
            printf("  [TFQMR-paired-GPU] device matvec unavailable; using GMRES\n");
            fflush(stdout);
        }
    }
