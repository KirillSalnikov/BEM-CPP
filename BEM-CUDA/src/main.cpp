...
static void print_usage(const char* prog) {
    ...
    printf("  --krylov TYPE   FMM iterative method: gmres, qmr2, cocr, block-cocg, gpu-gmres, tfqmr, bicgstab, bicgstab-rr, cgs-rr, gpu-adaptive, gpu-native, hybrid, or auto\n");
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--ka") == 0) {
            ...
            if (strcmp(krylov_kind, "gmres") != 0 &&
                ...
                strcmp(krylov_kind, "tfqmr") != 0 &&
                strcmp(krylov_kind, "qmr2") != 0 &&
                strcmp(krylov_kind, "cocr") != 0 &&
                strcmp(krylov_kind, "block-cocg") != 0 &&
                ...
                fprintf(stderr, "Error: --krylov must be gmres, qmr2, cocr, block-cocg, gpu-gmres, tfqmr, bicgstab, bicgstab-rr, cgs-rr, gpu-adaptive, gpu-native, hybrid, or auto\n");
                return 1;
            }
    ...
    std::complex<double> unknown_m_phase(1.0, 0.0);
    ...
    const bool qmr_phase_balance = bem_env_flag_enabled("BEM_QMR_PHASE_BALANCE") ||
                                   std::strcmp(krylov_kind, "qmr2") == 0 ||
                                   std::strcmp(krylov_kind, "cocr") == 0 ||
                                   std::strcmp(krylov_kind, "block-cocg") == 0;
    if (qmr_phase_balance) {
        // M_physical = -i M_internal/sM and the H row is multiplied by +i.
        // This is an exact change of variables that turns [[L,-K],[K,L]]
        // into a complex-symmetric block form when L^T=L and K^T=K.
        unknown_m_phase = std::complex<double>(0.0, -1.0);
        row_h_scale *= std::complex<double>(0.0, 1.0);
    }
    const std::complex<double> unknown_m_to_physical =
        unknown_m_phase / unknown_m_scale;
    ...
    // Short-recurrence probes are counterproductive for the strongly non-normal
    // high-frequency OBJ systems: a failed probe costs FMM matvecs before GMRES.
    // Keep an explicit escape hatch for controlled solver experiments.
    if (use_fmm && krylov_kind_set && std::strcmp(krylov_kind, "auto") == 0 &&
        is_obj && (ka >= 10.0 || mesh_quality.requires_remesh) &&
        !bem_env_flag_enabled("BEM_KRYLOV_AUTO_FORCE_PROBE")) {
        printf("  [Krylov-auto] high-frequency/quality-limited OBJ: using GMRES directly; "
               "set BEM_KRYLOV_AUTO_FORCE_PROBE=1 to benchmark short-recurrence methods.\n");
        krylov_kind = "gmres";
    }
    ...
    const bool use_tfqmr =
        use_fmm && krylov_env && strcmp(krylov_env, "tfqmr") == 0;
    const bool use_qmr2 =
        use_fmm && krylov_env && strcmp(krylov_env, "qmr2") == 0;
    const bool use_cocr =
        use_fmm && krylov_env && strcmp(krylov_env, "cocr") == 0;
    const bool use_block_cocg =
        use_fmm && krylov_env && strcmp(krylov_env, "block-cocg") == 0;
    const bool use_cocr_sym_prec = use_cocr && bem_env_flag_enabled("BEM_COCR_SYM_PREC");
    if (use_bicgstab || use_cgs_rr || use_tfqmr || use_qmr2 ||
        (use_cocr && !use_cocr_sym_prec) || use_block_cocg || use_krylov_auto || use_krylov_hybrid)
        no_prec = true;
    ...
    const bool use_gpu_gmres =
        use_fmm && !use_bicgstab && !use_cgs_rr && !use_tfqmr && !use_qmr2 && !use_cocr && !use_block_cocg && !use_krylov_auto && !use_krylov_hybrid &&
    ...
    const char* output_krylov_solver = use_krylov_auto ? "auto_best_short_recurrence_gmres_gpu" :
                                       ...
                                       (use_block_cocg ? "block_cocg_gpu_complex_symmetric" :
                                       (use_cocr ? "cocr_gpu_complex_symmetric" :
                                       (use_qmr2 ? "qmr2_gpu_complex_symmetric" :
                                       (use_tfqmr ? "tfqmr_gpu" :
    ...
    if (use_fmm) {
	    ...
	    fmm_op.unknown_m_phase = unknown_m_phase;
        ...
        if (coeffs_farfield_only) {
            ...
            if (bem_env_flag_enabled("BEM_OPERATOR_SYMMETRY_AUDIT")) {
                const bool block_audit = bem_env_flag_enabled("BEM_OPERATOR_SYMMETRY_BLOCK_AUDIT");
                const int probes = block_audit ? 4 :
                    std::max(1, bem_env_int("BEM_OPERATOR_SYMMETRY_PROBES", 3));
                std::vector<cdouble> sx(N2), sy(N2), sax(N2), say(N2);
                std::vector<cdouble> sax_single(N2), say_single(N2);
                double worst_rel = 0.0;
                for (int probe = 0; probe < probes; ++probe) {
                    for (int i = 0; i < N2; ++i) {
                        const double a = (double)(i + 1);
                        sx[i] = cdouble(std::sin(a * (0.173 + 0.019 * probe)),
                                        std::cos(a * (0.117 + 0.013 * probe)));
                        sy[i] = cdouble(std::cos(a * (0.137 + 0.017 * probe)),
                                        std::sin(a * (0.191 + 0.011 * probe)));
                        if (block_audit) {
                            const bool magnetic = i >= N;
                            if (probe == 1 && magnetic) {
                                sx[i] = cdouble(0.0);
                                sy[i] = cdouble(0.0);
                            } else if (probe == 2 && !magnetic) {
                                sx[i] = cdouble(0.0);
                                sy[i] = cdouble(0.0);
                            } else if (probe == 3) {
                                if (magnetic)
                    ...
                    fmm_op.matvec_batch2(sx.data(), sy.data(), sax.data(), say.data());
                    cdouble xtay(0.0, 0.0), ytax(0.0, 0.0);
                    double nx2 = 0.0, ny2 = 0.0, nax2 = 0.0, nay2 = 0.0;
                    for (int i = 0; i < N2; ++i) {
                        xtay += sx[i] * say[i];
                        ytax += sy[i] * sax[i];
                        nx2 += std::norm(sx[i]);
                    ...
                    const double scale = std::max(std::sqrt(nx2 * nay2) + std::sqrt(ny2 * nax2), 1e-300);
                    const double rel = std::abs(xtay - ytax) / scale;
                    worst_rel = std::max(worst_rel, rel);
                    const char* probe_name = !block_audit ? "random" :
                        (probe == 0 ? "full" : (probe == 1 ? "J-J" :
                         (probe == 2 ? "M-M" : "J-M")));
                    printf("  [OperatorSymmetry] probe=%d (%s) batch relative bilinear defect=%.6e\n",
                           probe + 1, probe_name, rel);

                    if (bem_env_flag_enabled("BEM_OPERATOR_SYMMETRY_COMPARE_SINGLE", true)) {
                        fmm_op.matvec(sx.data(), sax_single.data());
                        fmm_op.matvec(sy.data(), say_single.data());
                        cdouble xtay_single(0.0, 0.0), ytax_single(0.0, 0.0);
                        double naxs2 = 0.0, nays2 = 0.0, diff_ax2 = 0.0, diff_ay2 = 0.0;
                        for (int i = 0; i < N2; ++i) {
                            xtay_single += sx[i] * say_single[i];
                            ytax_single += sy[i] * sax_single[i];
                            naxs2 += std::norm(sax_single[i]);
                            nays2 += std::norm(say_single[i]);
                            diff_ax2 += std::norm(sax[i] - sax_single[i]);
                            diff_ay2 += std::norm(say[i] - say_single[i]);
                        }
                        const double single_scale = std::max(
                            std::sqrt(nx2 * nays2) + std::sqrt(ny2 * naxs2), 1e-300);
                        const double single_rel = std::abs(xtay_single - ytax_single) / single_scale;
                        const double batch_rel = std::max(
                            std::sqrt(diff_ax2 / std::max(naxs2, 1e-300)),
                            std::sqrt(diff_ay2 / std::max(nays2, 1e-300)));
                        worst_rel = std::max(worst_rel, single_rel);
                        printf("  [OperatorSymmetry] probe=%d single relative bilinear defect=%.6e, "
                               "batch-vs-single=%.6e\n",
                               probe + 1, single_rel, batch_rel);
                    }
                }
                printf("  [OperatorSymmetry] worst complex-transpose defect=%.6e (%d probes)\n",
                       worst_rel, probes);
                fflush(stdout);
                if (bem_env_flag_enabled("BEM_OPERATOR_SYMMETRY_AUDIT_ONLY")) {
                    fmm_op.cleanup();
                    return worst_rel <= bem_env_double("BEM_OPERATOR_SYMMETRY_MAX_REL", 1e-3) ? 0 : 2;
                }
            }
        }
        ...
        if (single_orient || sphere_orientation_shortcut) {
            ...
            if (unknown_m_to_physical != cdouble(1.0, 0.0)) {
                M_par_phys.resize(N);
                M_perp_phys.resize(N);
                for (int i = 0; i < N; i++) {
                    M_par_phys[i] = M_par[i] * unknown_m_to_physical;
                    M_perp_phys[i] = M_perp[i] * unknown_m_to_physical;
                }
            ...
            auto append_farfield_sample = [&](int oi, const Mat3& RT, double ca, double sa, double weight) {
                ...
                cdouble inv_s = unknown_m_to_physical;
                int bi = batch_count++;
